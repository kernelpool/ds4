...
/* Stage the matrix kernels' tiles in F32 (exact against the matvec kernels) or half. */
void ds4_gpu_set_exact_mm(int on);
...
/* The same routed experts for `n_tokens` tokens in one pass, at the one-token kernels'
 * precision: `x` is [n_tokens, in], `selected` / `weights` [n_tokens, n_expert], `out`
 * [n_tokens, out], and gate/up/mid/experts hold n_tokens * n_expert rows.  Only the
 * fused id pair+sum6 route carries several tokens, so it declines (returns 0) where
 * that route is unavailable -- quality mode, tensor parallel, SSD streaming -- and the
 * caller falls back to one token at a time.  No addend: the shared expert is added
 * afterwards. */
