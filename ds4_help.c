...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    if (full) {
        ...
        if (tool == DS4_HELP_DS4 || tool == DS4_HELP_AGENT || tool == DS4_HELP_SERVER) {
            ...
            opt(fp, c, "--dspark-confidence F", "Enable DSpark with confidence pruning threshold 0..1. Greedy/opportunistic default: Metal 0.6, CUDA/ROCm 0.7; exact sampling: 0.8 (DeepSeek V4.1 keeps the greedy default in both modes)");
...
static void print_sampling(FILE *fp, const help_colors *c, bool full) {
    ...
    opt(fp, c, "--reasoning-effort N", "DeepSeek V4.1 reasoning budget 1..100 while thinking. Default: 75, 100 with --think-max.");
    if (full) {
...
static void print_server_api(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--reasoning-effort N", "Default DeepSeek V4.1 reasoning budget 1..100 for requests that give none.");
