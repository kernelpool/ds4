/* Compare the split schedule with the complete original HC + query path. */
#include "../ds4_gpu.h"
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
int main(int argc,char **argv){
 ...
 const size_t qo=256,qbytes=32768ull*40*34,ho=qo+qbytes,hbytes=20480ull*24*2;
 ...
 q8 *qw=(q8*)((char*)map+qo);for(unsigned i=0;i<32768*40;i++){qw[i].d=0x2000+(rnd()%0x1800);for(unsigned j=0;j<32;j++)qw[i].q[j]=(int8_t)rnd();}
 ...
 const unsigned widths[]={5120,5120,24,24,32768,512};ds4_gpu_tensor *out[6];
 ...
 ds4_gpu_tensor *kv=ds4_gpu_tensor_alloc(512*4);CHECK(kv);
 ...
 float *r=malloc(20480*4),x[1280],p[4],k[512];CHECK(r);
 uint64_t words=0;
 for(draw=0;draw<draws+1000;draw++){
  const int repeat=draw>=draws;
  if(!repeat){for(unsigned i=0;i<20480;i++)r[i]=random_float();for(unsigned i=0;i<1280;i++)x[i]=random_float();for(unsigned i=0;i<4;i++)p[i]=random_float();for(unsigned i=0;i<512;i++)k[i]=random_float();}
  CHECK(ds4_gpu_tensor_write(kv,0,k,sizeof(k)));
  ...
  for(arm=repeat?1:0;arm<(repeat?2:3);arm++){
   ...
   unsigned pos=((repeat?draws-1:draw)*7919)%393216;bool compressed=((repeat?draws-1:draw)&1)!=0;
   if(arm==0){
    ...
    CHECK(ds4_gpu_dsv41_project_q(out[4],map,bytes,qo,1280,32768,qr,pos,compressed));
    CHECK(ds4_gpu_dsv41_rope_quantize(kv,out[5],0,512,pos,compressed,DS4_V41_FP8_E8M0));
   }else{
    CHECK(ds4_gpu_dsv41_deferred_collapse(out[0],out[1],res,pre,map,bytes,no,1e-6f));
    CHECK(ds4_gpu_dsv41_query_deferred_hc(out[4],out[2],out[3],qr,res,map,bytes,qo,ho,so,bo,pos,compressed,20,1e-6f,1e-6f,out[0],out[1],pre,no,kv,out[5],0));
   }
