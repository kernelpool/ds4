...
kernel void kernel_dsv41_index_score(
        ...
        ushort sg    [[simdgroup_index_in_threadgroup]],
        ushort nsg   [[simdgroups_per_threadgroup]]) {
    const uint i0 = (tgpig.x * (uint)nsg + (uint)sg) * DSV41_INDEX_SCORE_P;
    const uint t = tgpig.y;
    if (i0 >= args.n_scan || t >= args.rows) return;
    ...
    for (uint e = 0; e < idim4; e++) {
        ...
        for (uint p = 0; p < DSV41_INDEX_SCORE_P; p++) {
            dot[p] = fma(qv.x, k[p][4u * e], dot[p]);
            dot[p] = fma(qv.y, k[p][4u * e + 1u], dot[p]);
            dot[p] = fma(qv.z, k[p][4u * e + 2u], dot[p]);
            dot[p] = fma(qv.w, k[p][4u * e + 3u], dot[p]);
        }
    }
    for (uint e = idim4 * 4u; e < idim; e++) {
